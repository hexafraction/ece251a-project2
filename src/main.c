...
void init(void) {
	...
	// [P0MAT - Port 0 Match]$
	// $[P1 - Port 1 Pin Latch]
	P1 = P1_B4__LOW | P1_B5__LOW | P1_B6__LOW;
	// [P1 - Port 1 Pin Latch]$
	...
	// $[P1MAT - Port 1 Match]
	// [P1MAT - Port 1 Match]$
	P1MAT = P1MAT_B4__HIGH | P1MAT_B5__HIGH | P1MAT_B6__HIGH;
...
int main(void) {
	...
	IE_EA = 1;
	DISP_Init();
	renderAndWriteCenteredText(46, 1, "Stopwatch Timer");
	renderAndWriteCenteredText(64, 0, "Andrey Akhmetov");
	renderAndWriteCenteredText(74, 0, "Rafi Mueen");
	performCountdown();
	DISP_ClearAll();
	setupTimer();
	disableWdt();
	renderAndWriteCenteredText(5, 1, "Stopwatch Timer");
	renderAndWriteHorizontalLine(12, 115, 25);
	renderAndWriteHorizontalLine(12, 115, 53);
	renderAndWriteCenteredText(60, 0, "Lap						Time");
	lap1 = 0;
	lap2 = 0;
	lap3 = 0;
	while (1) {
		int timeInMilli = (int) getElapsedMillis();
		sprintf(str, "%02d:%02d:%02d.%d", timeInHours(timeInMilli),
				timeInMinutes(timeInMilli), timeInSeconds(timeInMilli),
				timeInMilliseconds(timeInMilli));
		renderAndWriteCenteredText(35, 0, str);
		sprintf(str, "1:	%02d:%02d:%02d.%d", timeInHours(lap1),
				timeInMinutes(lap1), timeInSeconds(lap1),
				timeInMilliseconds(lap1));
		renderAndWriteCenteredText(75, 0, str);
		sprintf(str, "2:	%02d:%02d:%02d.%d", timeInHours(lap2),
				timeInMinutes(lap2), timeInSeconds(lap2),
				timeInMilliseconds(lap2));
		renderAndWriteCenteredText(90, 0, str);
		sprintf(str, "3:	%02d:%02d:%02d.%d", timeInHours(lap3),
				timeInMinutes(lap3), timeInSeconds(lap3),
				timeInMilliseconds(lap3));
		renderAndWriteCenteredText(105, 0, str);
		//P1_B4 = timeInSeconds(timeInMilli) % 2;
		//P1_B5 = timeInSeconds(timeInMilli) % 2;
		if (!P0_B2) {
			if (!lP0B2) {
				if (timerStatus == 0) {
					start();
			...
			if (!lP0B3) {
				if (!timerStatus) {
					...
					if (!lap1) {
						...
						renderAndWriteCenteredText(120, 0, "FPGAs are better");
					}
				}
			}
			lP0B3 = true;
